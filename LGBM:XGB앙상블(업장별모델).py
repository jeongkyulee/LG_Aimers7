...
for step_ahead in range(1, PREDICT_DAYS+1):
    ...
    lgbm.fit(
        ...
        eval_metric='mae',  # 학습 모니터링/ES는 그대로 MAE 사용
...
# └ 내부(LGB/XGB) sMAPE-Softmax 가중 + 공용 vs 전용 블렌딩까지 계산
shop_models_lgb = {s:{} for s in SPECIAL_SHOPS}
shop_models_xgb = {s:{} for s in SPECIAL_SHOPS}
shop_step_blend = {s:{} for s in SPECIAL_SHOPS}       # 공용 vs 전용 블렌딩 가중(전용 비중)
shop_internal_weights = {s:{} for s in SPECIAL_SHOPS} # 전용 내부 LGB/XGB Softmax 가중 (wl_in, wx_in)
...
for shop in SPECIAL_SHOPS:
    ...
    for step_ahead in range(1, PREDICT_DAYS+1):
        ...
        if len(va_s_) == 0:
            continue
        X_va_s, y_va_s = va_s_[features], np.log1p(va_s_['target'].clip(lower=0))  # 검증 타깃(log)
        ...
        # ---- 전용 내부 앙상블: sMAPE-Softmax로 (LGB, XGB) 가중 산출 ----
        ...
        shop_internal_weights[shop][step_ahead] = (wl_in, wx_in)

        # 내부가중 적용한 전용 예측(검증용)
        preds_shop_val = wl_in*pred_lgb_val + wx_in*pred_xgb_val

        # ---- 공용 vs 전용 블렌딩 가중 (전용 비중) ----
        ...
        print(f"[{shop} step={step_ahead}] 내부 sMAPE LGB={s_lgb:.4f} XGB={s_xgb:.4f} -> wl_in={wl_in:.3f}, wx_in={wx_in:.3f} | "
              f"공용vs전용 sMAPE common={s_common:.4f} shop={s_shop:.4f} -> w_shop={w_shop:.3f}")

print("✅ 전용 모델 학습/내부가중/블렌딩 가중 산출 완료")
    ...
    for step_ahead in range(1, PREDICT_DAYS+1):
        ...
        for shop in SPECIAL_SHOPS:
            ...
            if (shop not in shop_models_lgb) or (step_ahead not in shop_models_lgb[shop]):
                continue
            if (shop not in shop_models_xgb) or (step_ahead not in shop_models_xgb[shop]):
                continue

            # 전용 내부 가중 (없으면 0.5/0.5)
            wl_in, wx_in = shop_internal_weights.get(shop, {}).get(step_ahead, (0.5, 0.5))

            X_test_s = X_test[mask]
            # 전용 예측(log→exp) 후 내부가중 적용
...
out_path = "/Users/jeong-kyu/Documents/LG_Aimers_7기/open/submission_plus_features_smape3(업장별 모델).csv"
